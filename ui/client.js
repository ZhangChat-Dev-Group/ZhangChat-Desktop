...
md.renderer.rules.text = function(tokens, idx) {
	...
	if (tokens[idx].content.indexOf('?') !== -1) {
		tokens[idx].content = tokens[idx].content.replace(/(^|\s)(\?)\S+?(?=[,.!?:)]?\s|$)/gm, function(match) {
			...
			return whiteSpace + '<a onclick="return verifyChannel()" href="' + channelLink + '">' + channelLink + '</a>';
		});
	}
...
function verifyChannel(){
	if (myChannel){
		alert('抱歉，由于技术的限制，您当前不能加入其他的频道，您可以再打开一个程序来加入新的频道')
		return false
	}else{
		return true
	}
	...
	ws.onopen = async function () {
		...
		if (!wasConnected) {
			if (location.hash) {
				...
				pushMessage({
					nick:'*',
					text:'请在下方的输入框中输入您的昵称：'
				})
				document.getElementById('chatinput').value = myNick
				var newNick
				await getInput().then((text) => {
					newNick = text
				})
				.catch(() => {
					shouldConnect = false
				...
				if (newNick !== null && newNick) {
					...
					shouldConnect = false;
					pushMessage({nick:'*',text:'您取消了此操作，程序将在五秒后退出。'})
					setTimeout(() => {
						window.close()
					},5000)
				}
		...
		if (myNick && shouldConnect) {
			pushMessage({nick:'*',text:'正在加入聊天室，请稍后。'})
			localStorageSet('my-nick', myNick);
			await getMurmur();
			//console.log(`murmur is: ${myMurmur}`)
			var sendMurmur=encode(myMurmur)
			send({ cmd: 'join', channel: channel, nick: myNick, client:'ZhangChatOnDesktop', murmur: sendMurmur.toString() });
		}
...
function pushMessage(args,cls = undefined) {    //cls指定messageEl添加什么classList
	...
	if (args.head){
		...
		imgEl.src = args.head === '/imgs/head.png' ? 'https://chat.zhangsoft.cf/imgs/head.png' : args.head
...
function pushVideo(url) {    //cls指定messageEl添加什么classList
	// Message container
	var messageEl = document.createElement('div');
...
function getInput_onKeyDown(e){
	if (!gettingInput){
		document.getElementById('chatinput').onkeydown = onKeyDown
		gettingInput = false
		onKeyDown(e)
		return
	}else{
		if (e.keyCode === 13){
			e.preventDefault();
			document.getElementById('chatinput').onkeydown = onKeyDown
			gettingInput.res(e.target.value)
			gettingInput = false
			e.target.value = ''
...
function getInput(){
	return new Promise((res,rej) => {
		...
		document.getElementById('chatinput').onkeydown = getInput_onKeyDown
	})
}
...
$('#set-video').onclick = async function() {
	if (document.getElementById('chatinput').value){
		return pushMessage({nick:'!',text:'您的输入框里还有未发送的信息，请清除或发送以后再试'})
	}
    var newVideo
	pushMessage({nick:'*',text:'请在下方的输入框中输入新视频的地址（如果要删除视频请设置为nothing）：'})
	await getInput().then((text) => {
		newVideo = text
	})
	.catch(() => {
		pushMessage({nick:'!',text:'糟了...出现了未知错误'})
	})
	if (newVideo === null || !newVideo){
		return pushMessage({
			nick:'!',
			text:'您取消了设置视频'
		})
...
$('#set-head').onclick = async function () {
	if (document.getElementById('chatinput').value){
		return pushMessage({nick:'!',text:'您的输入框里还有未发送的信息，请清除或发送以后再试。'})
	}
	pushMessage({nick:'*',text:'请在下方的输入框中输入您的头像的地址：'})
	document.getElementById('chatinput').value = localStorageGet('head') || ''
	var newHead
	await getInput().then((text) => {
		newHead = text
	})
